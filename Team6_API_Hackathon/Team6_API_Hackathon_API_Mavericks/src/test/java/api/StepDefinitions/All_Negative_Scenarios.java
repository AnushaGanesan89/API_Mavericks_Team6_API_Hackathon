...
public class All_Negative_Scenarios {
	...
	public void user_sends_https_request_and_request_body_without_patientinfo_and_attaches_only_the_file_tries_to_create_a_patient_records_from_the_excel_sheet_and(String SheetName, Integer RowNumber) throws InvalidFormatException, IOException, ParseException {
		ExcelReader reader=new ExcelReader();
		List<Map<String,String>> testData=reader.getData(".\\TestData\\API_MAVERICKS_RESTASSURED_DATA.xlsx", SheetName);
		
		String Allergy=testData.get(RowNumber).get("Allergy");
		String FoodCategory=testData.get(RowNumber).get("FoodCategory");
		ntc.TestPostPatient_noPatientInfo(Allergy, FoodCategory);
	...
	public void user_receives_bad_request_status_created_and_patientid_is_not_created(Integer int1) {
		 int1= ntc.verify_post_nopatientinfo_status();
		    int Expected=400;
		    Assert.assertEquals(int1, Expected);
	...
	public void user_sends_https_request_and_request_body_with_mandatory_and_additional_fields_to_update_a_patient_records_with_invalid_patient_id_from_the_excel_sheet_and(String SheetName, Integer RowNumber) throws InvalidFormatException, IOException, ParseException {
		ExcelReader reader=new ExcelReader();
		List<Map<String,String>> testData=reader.getData(".\\TestData\\API_MAVERICKS_RESTASSURED_DATA.xlsx", SheetName);
		...
		ntc.Update_Patient(Allergy,FoodCategory);
	}
	...
	public void user_recieves_bad_request_is_created_and_update_failed(Integer int1) {
		 int1= ntc.verify_put_patient_status();
		    int Expected=404;
		    Assert.assertEquals(int1, Expected);
	...
	public void user_sends_https_request_with_invalid_patient_id(Integer int1) {
		int1=Env_Var.invalid_patientId;
		ntc.Get_PatientDetails_UsingID(int1);
	...
	public void user_recieves_not_found_and_reports_failed_to_be_displayed(Integer int1) {
		int1=ntc.verify_get_patient_usingid_status();
		int expected= 404;
		Assert.assertEquals(int1, expected);
	...
	public void user_sends_https_request_with_invalid_fileid(String string) {
		...
		string= Env_Var.invalid_fileid;
		ntc.Get_PatientFiles_UsingFileID(string);
	...
	public void user_recieves_not_found_with_no_patient_test_report_details(Integer int1) {
		int1=ntc.verify_get_patient_file_status();
		int expected= 404;
		Assert.assertEquals(int1, expected);
	...
	public void user_sends_https_request_with_invalid_morbidity_test_name(String string) {
		string=Env_Var.invalid_morbiditytestname;
		ntc.Get_MorbidityDetails_UsingName(string);
	...
	public void user_recieves_bad_request_with_no_details_of_morbidity_test_name_getting_displayed(Integer int1) {
		 int1=ntc.verify_get__morbiditybyName_status();
		    int expected= 404;
			Assert.assertEquals(int1, expected);
